		...
		function ServerHold:Server(text, img)
			...
			function ChannelHold:Channel(text)
				...
				function ChannelContent:Toggle(text, default, callback)
					...
					function toggle_func:toggle(bool, call_callback)
						if bool then
						...
						if call_callback then
							pcall(callback, bool, toggle_func)
						end
					end

					function toggle_func:set_disable(bool)
						is_disabled = bool
					end

					toggle_func:toggle(toggled, true)
...
local RECORD_FOLDER_NAME 		= "ASTD_RECORDS"
local SETTINGS_FILE_NAME 		= "settings.json"
local MAP_ENDED_CONTROLLER_NAME = "RemoteFunction"

local save_file_path = RECORD_FOLDER_NAME.."\\"..SETTINGS_FILE_NAME

local AUTO_UPGRADING_SELECTED_INTERVAL  = 1.5
...
local autofarm = window:Server("Auto Farm Section", "http://www.roblox.com/asset/?id=11579310982")
...
local function save_settings()
	local settings = {
		auto_replay 	 = is_auto_replaying,
		auto_x2 		 = is_auto_timelapsing,
	...
	writefile(save_file_path, HttpService:JSONEncode(settings))
end

local function load_settings()
	debug("Loading Data")
	local data = isfile(save_file_path) and HttpService:JSONDecode(readfile(save_file_path)) or {}
	if data.auto_replay ~= nil then
		is_auto_replaying = data.auto_replay
	end

	if data.auto_x2 ~= nil then
		is_auto_timelapsing = data.auto_x2
	end

	if data.play_record ~= nil then
		is_record_playing = data.play_record
	end

	if data.anti_afk ~= nil then
		anti_afk = data.anti_afk
	end

	if data.auto_upgrade_all ~= nil then
	...
	if data.auto_extreme ~= nil then
		is_auto_voting_extreme = data.auto_extreme
	end

	debug("Data Loaded: "..HttpService:JSONEncode(data))
	is_loaded = true
end

local function exist(list, item)
	local exist = false
...
local function GetTowerAtPosition(tower_name, position)
	local tower
	for _, v in ipairs(workspace.Unit:GetChildren()) do
		if v.Name == tower_name and v.HumanoidRootPart.Position == position then
			tower = v
			break
		end
	end
...
autofarm_tab:Toggle("Auto Replay", is_auto_replaying, function(bool)
	is_auto_replaying = bool
	save_settings()
end)
...
do		-- Auto upgrade	
	...
	auto_upgrade_selected_button = autofarm_tab:Toggle("Auto upgrade selected tower", auto_upgrading_selected, function(bool)
		...
		if is_ingame and bool then
			-- disable upgrade all button
			auto_upgrade_all_button:toggle(false, true)
			auto_upgrade_all_button:set_disable(true)

			coroutine.wrap(function()
				while auto_upgrading_selected do
					...
					wait(AUTO_UPGRADING_SELECTED_INTERVAL)
				end
			end)()
		else
			auto_upgrade_all_button:set_disable(false)
		end
	end)
...
do		-- Game record
	...
	local namecall_original, newindex_original = game_metatable.__namecall, game_metatable.__newindex
	...
	local record_gameplay_button = autofarm_tab:Toggle("Record gameplay", is_recording, function(enabled)
		...
		if not enabled then
			if #call > 0 then
				save_recorded_gameplay()
			else
				debug("record has not been saved | recorded length: "..tostring(#call))
			end
		end
	end)

	game_metatable.__namecall = newcclosure(function(self, _arg1, ...)
		if is_ingame and is_recording then
			local method = getnamecallmethod() 
			local script = getcallingscript()
			if (method == "FireServer" or method == "InvokeServer") then
				local args = ...
				local _action = _arg1
				if _action == "Summon" then
					local copied_args = {}
		...
		return namecall_original(self, _arg1, ...) 
	end)
	...
	game_metatable.__newindex = newcclosure(function(self, method, listener)
		...
		return newindex_original(self, method, listener)
	end)
	...
	autofarm_tab:Toggle("Play recorded gameplay", is_record_playing, function(enabled, control)
		...
		if is_ingame and enabled and isfile(RECORD_FOLDER_NAME.."\\"..tostring(ReplicatedStorage.Map.Value)..".json") then
			...
			coroutine.wrap(function()
				while is_record_playing and action_queue <= #actions do
					local current_queue = actions[action_queue]
					if current_queue.action == "Summon" then
						if LocalPlayer.Money.Value >= cached.getstat(current_queue.info.Unit).Cost then
							remote:FireServer(current_queue.action, current_queue.info)
							action_queue = action_queue + 1
						end
					elseif current_queue.action == "Upgrade" then
						local tower = GetTowerAtPosition(current_queue.info.Unit, current_queue.info.cframe.Position)
						if tower then
							local next_upgrade_stats = calculator.GetAddedStatsForNextUpgrade(tower):expect()
							if next_upgrade_stats then
								if LocalPlayer.Money.Value >= next_upgrade_stats.Cost then
									game.ReplicatedStorage.Remotes.Server:InvokeServer(current_queue.action, tower);
									action_queue = action_queue + 1
								end
							else
								-- skip action
					...
					if action_queue > #actions then
						debug("Done playing record")
						break
					end

					wait(RECORD_PLAYBACK_INTERVAL)
				end
			end)()
		end
		save_settings()
...
misc_tab:Toggle("Anti-AFK", anti_afk, function(bool)
	anti_afk = bool
	if bool then
		debug("AntiAFK - Starting")

		local VirtualUser=game:service'VirtualUser'
		anti_afk_connection = game:service('Players').LocalPlayer.Idled:connect(function()
			debug("Idle detected")
			VirtualUser:CaptureController()
			VirtualUser:ClickButton2(Vector2.new())
		end)
		
		debug("AntiAFK - Started")
	else
		if anti_afk_connection then
			anti_afk_connection:Disconnect()
			debug("AntiAFK - Stopped")
		end
	end
end)
